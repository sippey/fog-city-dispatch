...
export default function ShowcasePage() {
  ...
  // When all cards are gone, transition directly to game
  useEffect(() => {
    if (cardStack.length === 0 && isPreloaded && isAnimatingAway && !showGame) {
      // Skip the title screen entirely, go straight to game
      setTimeout(() => {
        setShowGame(true)
      }, 500) // Small delay after last card disappears
    }
  }, [cardStack.length, isPreloaded, isAnimatingAway, showGame])
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      >
        ...
        <div className="relative z-20 min-h-screen flex items-center justify-center">
          ...
          {/* Show sparkle briefly when cards are gone */}
          <motion.div
            className="absolute inset-0 flex items-center justify-center"
            animate={{ 
              opacity: cardStack.length === 0 && isPreloaded && !showGame ? 1 : 0 
            }}
      ...
      {/* Game content - only render when showGame is true */}
      {showGame && (
        <motion.div
          className="absolute inset-0"
          initial={{ opacity: 0 }}
