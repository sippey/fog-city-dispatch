...
export default function Card({ card }: CardProps) {
  ...
  const getImagePath = () => {
    if (card.imageFile) {
      return `/images/cards/${card.imageFile}.png`
    }
    // Auto-match by headline
    return `/images/cards/${card.headline}.png`
  }

  const getAudioPath = () => {
    if (card.audioFile) {
      return `/audio/cards/${card.audioFile}.mp3`
    }
    // Auto-match by headline
    return `/audio/cards/${card.headline}.mp3`
  }

  const hasImage = () => {
    return card.imageFile || true // Assume image exists, Next.js will handle fallback
  }

  const hasAudio = () => {
    // Check if this card matches known audio files
    const audioFiles = [
    ...
    return card.audioFile || audioFiles.includes(card.headline)
  }

  return (
    <div className={`bg-white rounded-3xl overflow-hidden shadow-lg border transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${
      card.isPowerup 
      ...
      >
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {hasImage() && (
              <span className="inline-flex items-center px-3 py-1 rounded-2xl text-xs font-semibold text-white bg-gradient-to-r from-green-500 to-green-600">
                🖼️ Artwork
              </span>
            )}
            {hasAudio() && (
              <span className="inline-flex items-center px-3 py-1 rounded-2xl text-xs font-semibold text-white bg-gradient-to-r from-pink-500 to-pink-600">
                🎵 Audio
              </span>
            )}
            {card.isPowerup && (
            ...
            <div className="mb-6">
              ...
              <div className="relative w-full max-w-md mx-auto rounded-2xl overflow-hidden shadow-lg">
                <Image
                  ...
                  onError={(e) => {
                    // Hide the image section if image fails to load
                    const imgElement = e.target as HTMLImageElement
                    const artworkSection = imgElement.closest('.mb-6')
                    if (artworkSection) {
                      (artworkSection as HTMLElement).style.display = 'none'
                    }
                  }}
                />
              </div>
          ...
          {card.voiceScript && (
            <div className="mb-8 bg-gradient-to-r from-amber-100 to-amber-200 p-5 rounded-2xl border-l-4 border-amber-500 border border-amber-300">
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-blue-600 text-lg font-bold uppercase tracking-wide">
                  Voice Script:
                </h4>
                {hasAudio() && (
                  <button
              ...
              <p className="text-amber-900 leading-relaxed font-medium italic">&ldquo;{card.voiceScript}&rdquo;</p>
              {hasAudio() && (
                <audio
                  ref={audioRef}
                  src={getAudioPath()}
              ...
              <div className="bg-gradient-to-r from-amber-100 to-amber-200 p-6 rounded-3xl border-4 border-amber-500">
                ...
                <div className="text-center p-3">
                  <span className="text-amber-900 text-lg font-semibold italic">
                    &ldquo;{(card.responses as Record<string, { outcome?: string }>).accept?.outcome || 'Feeling recharged and ready!'}&rdquo;
                  </span>
                </div>
              </div>
            ) : (
                ...
                <div className="mt-8 pt-6 border-t-2 border-gray-200">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl border border-gray-200 border-l-4 border-l-gray-500 hover:-translate-x-1 transition-transform">
                      ...
                      <span className="flex-1 text-gray-700 italic text-sm font-medium">&ldquo;{card.responses.ignore!.outcome}&rdquo;</span>
                    </div>
                    <div className="flex items-center gap-3 p-4 bg-blue-50 rounded-2xl border border-blue-200 border-l-4 border-l-blue-500 hover:-translate-x-1 transition-transform">
                      ...
                      <span className="flex-1 text-gray-700 italic text-sm font-medium">&ldquo;{card.responses.basic!.outcome}&rdquo;</span>
                    </div>
                    <div className="flex items-center gap-3 p-4 bg-red-50 rounded-2xl border border-red-200 border-l-4 border-l-red-500 hover:-translate-x-1 transition-transform">
                      ...
                      <span className="flex-1 text-gray-700 italic text-sm font-medium">&ldquo;{card.responses.maximum!.outcome}&rdquo;</span>
                    </div>
                  </div>
                </div>
              </>
